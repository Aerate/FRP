...
-- Angepasste Version von Manuel, mit verallgemeinerten Levels.
-- Wird bald in der Standardbibliothek sein, dann ist diese Datei obsolet.
...
map : ∀ {c ℓ₁ ℓ₂} {C : Container c} {X : Set ℓ₁} {Y : Set ℓ₂} → (X → Y) → ⟦ C ⟧ X → ⟦ C ⟧ Y
...
APred : ∀ {c ℓ₁ ℓ₂} {C : Container c} {X : Set ℓ₁} →
    (X → Set ℓ₂) → ⟦ C ⟧ X → Set (c ⊔ ℓ₂)
APred P (s , f) = ∀ p → P (f p)
...
E : ∀ {c ℓ} {C : Container c} → ⟦ C ⟧ (Set ℓ) → Set (c ⊔ ℓ)
E (s , f) = ∃ λ p → f p

EPred : ∀ {c ℓ₁ ℓ₂} {C : Container c} {X : Set ℓ₁} →
    (X → Set ℓ₂) → (⟦ C ⟧ X → Set (c ⊔ ℓ₂))
EPred P (s , f) = ∃ λ p → P (f p)
